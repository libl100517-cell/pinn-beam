...
def main():
    ...
    # Elastic concrete (huge eps_co so Popovics stays linear in working range)
    # SmoothConcrete with eps_co=-0.1 → n = Ec*0.1/fc = 100 → nearly linear
    concrete = SmoothConcrete(fc=cfg.fc, Ec=cfg.Ec, eps_co=-0.1)
    steel = BilinearSteel(fy=cfg.fy, Es=cfg.Es, b=cfg.steel_b)
    print(f"  Concrete: ELASTIC (SmoothConcrete eps_co=-0.1, n={concrete.n:.0f})")
